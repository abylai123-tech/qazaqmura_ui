...
async function getContractors() {
  loading.value = true
  try {
    const request = `/v2/inventory?active=1&page=${page.value}`
...
const writeOffBooks = async () => {
  const body = {
    book_school_id: items.value[0].book_school_id,
    inventories: selected.value
  }
  const response = await api.patchData('/v1/book/inventory/deprecated', body)
  if (response.data) {
    snackbar.value = true
      ...
      <v-col cols="12">
        ...
        >
          <template v-slot:actions>
            <v-btn class="my-auto mr-4" color="primary" variant="flat" @click="writeOffBooks"
              >Списать выбранное
            </v-btn>
          </template>
        </FilterBlock>
